...
var kON_AUTOSERVICE_BY_ID__AUTO_SERVICE_BY_ID_STORE_PRIORITY =  sc.kON_AUTOSERVICE_BY_ID__AUTO_SERVICE_BY_ID_STORE_PRIORITY; //меньше дефолтной
...
var sort_results_ = (results, center, bounds) => {
  ...
  var marker_visible = state_.autoservice_data.get('markers').find(marker => marker.get('balloon_visible') === true );
...
var cncl_ = [
  ...
  main_dispatcher
  .on(event_names.kON_AUTOSERVICE_BY_ID_CHANGE_PAGE, page_num => {
  ...
  main_dispatcher
  .on(event_names.kON_AUTOSERVICE_BY_ID_CHANGE_ITEMS_PER_PAGE, items_per_page => {  
  ...
  .on(event_names.kON_AUTOSERVICE_BY_ID_DATA_LOADED, (autoservice_data) => {  
    ...
    state_.results_sorted_cursor
    .update( () => 
      state_.autoservice_data.get('results')
        .map( r => 
  ...
  main_dispatcher
  .on(event_names.kON_AUTOSERVICE_BY_ID_CLOSE_ALL_BALLOON, () => { 
    if(!state_.autoservice_data) return;
  ...
  .on(event_names.kON_AUTOSERVICE_BY_ID_TOGGLE_BALLOON, id => { 
    ...
    var index  = state_.autoservice_data.get('markers').findIndex(marker => {
  ...
  //---------------------------------------------------------------------
  main_dispatcher
  .on(event_names.kON_AUTOSERVICE_BY_ID_MARKER_HOVER, (id, hover_state, options) => { 
  ...
  //------------------------------------------------------------------------------------
  main_dispatcher
  .on(event_names.kON_AUTOSERVICE_BY_ID_MAP_BOUNDS_CHANGED_BY_USER, (center, zoom, bounds) => { 
